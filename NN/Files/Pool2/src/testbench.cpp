...
int main()
{
    ...
    ifstream inp_file("/primary/HLS/AlexNet-FPGA-Implementation/Pool2/out_conv2.txt");
    DataType *inp_image;
    inp_image = (DataType *)malloc( INP_IMG_SIZE_Pool2 * INP_IMG_SIZE_Pool2 * INP_IMG_CHAN_Pool2 * sizeof(DataType));
	if(inp_file.is_open())
	{
		cout << "can open the text file" << endl;


		for (int i=0; i<INP_IMG_SIZE_Pool2 * INP_IMG_SIZE_Pool2 * INP_IMG_CHAN_Pool2; i++)
		{
  ...
  DataType *out_image = (DataType *)malloc(OUT_IMG_SIZE_Pool2 * OUT_IMG_SIZE_Pool2 * INP_IMG_CHAN_Pool2 * sizeof(DataType));
  ...
  ofstream data("/primary/HLS/AlexNet-FPGA-Implementation/Pool2/out_image.txt");
  for (int k = 0; k < OUT_IMG_SIZE_Pool2*OUT_IMG_SIZE_Pool2*INP_IMG_CHAN_Pool2; k++)
    {
      data << out_image[k] << "\n";
    }
  ...
  const DataType out_img[] = {
      ...
      DataType big_diff = 0;
      DataType diff[OUT_IMG_SIZE_Pool2*OUT_IMG_SIZE_Pool2*INP_IMG_CHAN_Pool2];
  for (int i=0; i<OUT_IMG_SIZE_Pool2*OUT_IMG_SIZE_Pool2*INP_IMG_CHAN_Pool2; i++){
