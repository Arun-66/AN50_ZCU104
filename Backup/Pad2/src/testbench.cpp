...
int main()
{
    ...
    ifstream inp_file("/primary/HLS/AlexNet-FPGA-Implementation/Pad2/out_norm2.txt");
    DataType *inp_image;

    inp_image = (DataType *)malloc( INP_IMAGE_SIZE_Pad2 * INP_IMAGE_SIZE_Pad2 * INP_IMAGE_CHANNEL_Pad2 * sizeof(DataType));
	if(inp_file.is_open())
	{
		cout << "can open the text file" << endl;


		for (int i=0; i<INP_IMAGE_SIZE_Pad2 * INP_IMAGE_SIZE_Pad2 * INP_IMAGE_CHANNEL_Pad2; i++)
		{
	...
	DataType *out_image = (DataType *)malloc(OUT_IMAGE_SIZE_Pad2 * OUT_IMAGE_SIZE_Pad2 * OUT_IMAGE_CHANNEL_Pad2 * sizeof(DataType));
  ...
  ofstream data("/primary/HLS/AlexNet-FPGA-Implementation/Pad2/inp_conv3.txt");
  for (int k = 0; k < OUT_IMAGE_SIZE_Pad2*OUT_IMAGE_SIZE_Pad2*OUT_IMAGE_CHANNEL_Pad2; k++)
    {
      data << out_image[k] << "\n";
