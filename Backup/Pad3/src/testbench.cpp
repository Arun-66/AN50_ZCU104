...
int main()
{
    ...
    ifstream inp_file("/primary/HLS/AlexNet-FPGA-Implementation/Pad3/out_conv3.txt");
    DataType *inp_image;

    inp_image = (DataType *)malloc( INP_IMAGE_SIZE_Pad3 * INP_IMAGE_SIZE_Pad3 * INP_IMAGE_CHANNEL_Pad3 * sizeof(DataType));
	if(inp_file.is_open())
	{
		cout << "can open the text file" << endl;


		for (int i=0; i<INP_IMAGE_SIZE_Pad3 * INP_IMAGE_SIZE_Pad3 * INP_IMAGE_CHANNEL_Pad3; i++)
		{
	...
	DataType *out_image = (DataType *)malloc(OUT_IMAGE_SIZE_Pad3 * OUT_IMAGE_SIZE_Pad3 * OUT_IMAGE_CHANNEL_Pad3 * sizeof(DataType));
  ...
  ofstream data("/primary/HLS/AlexNet-FPGA-Implementation/Pad3/inp_conv4.txt");
  for (int k = 0; k < OUT_IMAGE_SIZE_Pad3*OUT_IMAGE_SIZE_Pad3*OUT_IMAGE_CHANNEL_Pad3; k++)
    {
      data << out_image[k] << "\n";
